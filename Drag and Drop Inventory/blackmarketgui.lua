...
function BlackMarketGui:mouse_pressed(button, x, y)
	...
	if self._enabled and not self._renaming_item and self._highlighted and button == Idstring("0") and self._tabs[self._highlighted]:inside(x, y) == 1 then
		local ctg = self._slot_data.category
		if (ctg == "masks" and self._slot_data.slot ~= 1) or ctg == "primaries" or ctg == "secondaries" then
		...
		elseif self._tabs[self._highlighted]:inside(x, y) == 1 then
			...
			if DragDropInventory.dragging then
				...
				if DragDropInventory.slot_data.bitmap_texture then
					DragDropInventory.item_bmp = DragDropInventory.item_bmp or self._panel:bitmap({
						texture = DragDropInventory.slot_data.bitmap_texture,
						layer = tweak_data.gui.MOUSE_LAYER - 50,
					})
					DragDropInventory.item_bmp:set_center(x, y)
				end
			end
		end
		grab = true
...
function BlackMarketGui:mouse_released(button, x, y)
	if button == Idstring("0") then
		...
		if DragDropInventory.item_bmp then
			self._panel:remove(DragDropInventory.item_bmp)
			DragDropInventory.item_bmp = nil
		end
		DragDropInventory.dragging = false
